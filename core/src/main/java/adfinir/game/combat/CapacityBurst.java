 ...
 * autour d'un point de cast. Lit getActiveEffect()/getActiveModifiers() (et non
 * mainEffect/modifiers bruts) pour refléter les CapacityEffectMod socketés.
 */
public class CapacityBurst {
    ...
    /** Calcule les paramètres du burst à partir de la capacité équipée (effet + modificateurs actifs). */
    public static BurstParams resolve(Capacity cap) {
        BurstParams p = new BurstParams();
        CapacityEffect effect = cap.getActiveEffect();
        p.damage = effect.baseDamage * cap.rarity.statMultiplier;
        p.radius = effect.radius;
        p.cooldown = 3.0f;
        p.staminaCost = 30f;

        for (CapacityModifier m : cap.getActiveModifiers()) {
            switch (m.type) {
                case EXPLOSION:
                case BOUNCE:
                case RICOCHET:
